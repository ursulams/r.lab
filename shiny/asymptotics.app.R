...
ui <- shinyUI(
  fluidPage(theme = shinytheme("united"),
            ...
            h4("ursula kaczmarek (", a(href = "https://github.com/ursulakaczmarek/r.lab/tree/master/shiny", "github"), ")"),
            fluidRow(
              column(4,
...
output$sum.plot <- renderPlot({
    ...
    df <- data.frame(xbar = c(apply(matrix(sample(0:1, sn*50, replace = TRUE), sn), 1, cfunc, 50)))
    g <- ggplot(df, aes(x = xbar, alpha = 0.5)) +
